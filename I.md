Here is the Aegis Quick Start guide, designed with the clean, developer-centric aesthetic of OpenAI’s documentation.
Quickstart
...
| compute | Abstracted resource envelope (e.g., standard.cpu.xs). |
| region | The physical cloud location for deployment. |
| auth | The access policy (e.g., bearer, public). |
| timeout | Execution SLA in seconds. |
| scale | Autoscaling boundaries (min/max). |
Local Development
